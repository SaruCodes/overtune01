<x-layouts.layout titulo="Nuevo Disco">
    <div class="flex flex-col justify-center items-center min-h-screen bg-violet-100 py-12">
        ...
        <form action="{{ route('discos.store') }}" method="POST" class="w-full max-w-4xl bg-white shadow-lg rounded-lg p-8" enctype="multipart/form-data">
            ...
            <div class="max-h-96 overflow-y-auto px-4 text-sm relative">
                ...
                <div x-data="{ generos: {} }" class="grid grid-cols-1 gap-4">
                    @foreach(config('generos') as $genero)
                        <div class="flex items-center gap-3 border-b py-2">
                            ...
                            <template x-show="generos['{{ $genero }}']">
                                <div>
                                    <!-- Campo para Subgénero -->
            ...
            <div class="flex justify-end space-x-4 mt-8">
                <button type="submit" class="btn btn-sm btn-success px-6 py-2 bg-green-600 text-white rounded-md shadow-md hover:bg-green-700">
                    Guardar
                </button>
                <a href="{{ route('discos.index') }}" class="btn btn-sm btn-secondary px-6 py-2 bg-gray-600 text-white rounded-md shadow-md hover:bg-gray-700">
                    Cancelar
                </a>
            </div>
        </form>
